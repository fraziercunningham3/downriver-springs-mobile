...
let metroPort = Number(process.env.EXPO_BUILD_METRO_PORT) || null;
...
async function selectMetroPort() {
  if (metroPort) return metroPort;

  const probe = net.createServer();
  await new Promise((resolve, reject) => {
    probe.once('error', reject);
    probe.listen(0, '127.0.0.1', () => {
      metroPort = probe.address().port;
...
function metroUrl(pathname = '') {
  if (!metroPort) {
    throw new Error('Metro port has not been selected.');
  }
